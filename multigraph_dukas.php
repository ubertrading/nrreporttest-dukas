...
<head>
    ...
    <title>Dukascopy Multi Chart - <?php echo htmlspecialchars($news_title); ?></title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="info-bar">
        <h2><span class="badge">DUKASCOPY</span><?php echo htmlspecialchars($news_title); ?>
            <?php if ($event_datetime !== ''): ?>
                <span
                    style="font-size: 14px; color: #aaa; margin-left: 15px;"><?= htmlspecialchars($event_datetime) ?></span>
            <?php endif; ?>
        </h2>
        <div class="data-row">
            <?php if ($forecast_avg !== ''): ?>
    ...
    <div class="charts-grid">
        <?php foreach ($instruments as $symbol): ?>
            ...
            $iframe_url = 'graph_dukas.php?' . http_build_query($params);
            ?>
            <div class="chart-panel"
                onclick="expandChart('<?= htmlspecialchars(trim($symbol)) ?>', '<?= htmlspecialchars($iframe_url) ?>')">
