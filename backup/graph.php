...
function get_data($symbol, $timestamp, $from, $to, $timeframe, $retry = 0)
{
  ...
  ob_start();
  $cmd = "/usr/bin/python3 /home/jlippuner/quantflash/download_data.py " . $symbol . " " .
    $timestamp . " " . $timeframe;
...
if ($r['status'] == 'OK') {
  ...
  if (!empty($data)) {
    ...
    foreach ($data as $row) {
      ...
      $fxcmtimestamp = substr($datetimefxcm->format('Y-m-d H:i:s.u'), 0, -3);
      $datax[] = "'" . $fxcmtimestamp . "'";
...
<body>
  ...
  <div style="margin: 10px 0; padding: 10px 15px; background: #f0f0f0; border-radius: 6px; font-family: sans-serif;">
    <h2 style="margin: 0 0 6px 0;"><?php echo htmlspecialchars($news_title); ?></h2>
