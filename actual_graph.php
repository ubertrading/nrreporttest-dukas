...
foreach ($data as $evTimeStr => $row) {
    ...
    $actuals[] = $row['val'];
    $colors[] = $row['val'] >= 0 ? 'rgba(46, 125, 50, 0.8)' : 'rgba(198, 40, 40, 0.8)';
    $times[] = $row['time'];
}
...
<head>
  ...
  <title>Actual History: <?php echo htmlspecialchars($news_title); ?></title>
...
<body>
  ...
  <h2><?php echo htmlspecialchars($news_title); ?> - Historical Actuals</h2>
  ...
  <script>
    ...
    function updateChartScale() {
        ...
        if (minVal !== '') {
            actualChart.options.scales.y.min = parseFloat(minVal);
        } else {
            delete actualChart.options.scales.y.min;
        }
        ...
        actualChart.data.labels = filteredLabels;
        actualChart.data.times = filteredTimes;
        actualChart.data.datasets[0].data = filteredData;
        actualChart.data.datasets[0].backgroundColor = filteredColors;
        actualChart.data.datasets[0].borderColor = filteredColors.map(c => c.replace('0.8', '1'));
    ...
    function resetChartScale() {
        ...
        // Reset data
        actualChart.data.labels = [...origLabels];
        actualChart.data.times = [...origTimes];
        actualChart.data.datasets[0].data = [...origData];
        actualChart.data.datasets[0].backgroundColor = [...origColors];
        actualChart.data.datasets[0].borderColor = origColors.map(c => c.replace('0.8', '1'));
