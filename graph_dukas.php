...
// Dukascopy does not need instrument_slash_map
...
function get_dukas_data($symbol, $timestamp, $from, $to, $timeframe)
{
  debug_to_console("called get_dukas_data: " . $timeframe . " for " . $symbol . " at " .
    $timestamp . " from " . $from->format("Y-m-d H:i:s") . " to " . $to->format("Y-m-d H:i:s"));

  // Remove slash from symbol for Dukascopy
  $duka_symbol = str_replace("/", "", $symbol);

  $cmd = "python3 " . __DIR__ . "/dukascopy_data.py " . escapeshellarg($duka_symbol) . " " .
    escapeshellarg($timestamp) . " " . escapeshellarg($timeframe) . " 2>&1";
  debug_to_console("CMD: " . $cmd);
  $out = shell_exec($cmd);
  debug_to_console("Response length: " . strlen($out));
  debug_to_console("Response: " . $out);
  $res = json_decode($out, true);

  if ($res === null) {
    return ["status" => "ERROR", "data" => "Failed to parse JSON: " . substr($out, 0, 500)];
  }
  return $res;
}
...
$dateTime = DateTime::createFromFormat('Y-m-d H:i:s.u', $_GET['datetime'], $timezoneutc);
...
$dt_from = DateTime::createFromFormat('Y-m-d H:i:s.u', $_GET['datetime'], $timezoneutc);
$dt_from->modify('-60 seconds');
$time_tick_start = $dt_from->format('Uv');

$dt_to = DateTime::createFromFormat('Y-m-d H:i:s.u', $_GET['datetime'], $timezoneutc);
...
$dt_sched = DateTime::createFromFormat('Y-m-d H:i:s.u', $scheduled, $timezoneutc);
$dt_recv = DateTime::createFromFormat('Y-m-d H:i:s.u', $_GET['datetime'], $timezoneutc);
...
$r = get_dukas_data($symbol, $news_timestamp, $dt_from, $dt_to, "t1");

$resolved_symbol = $r['resolved_symbol'] ?? $symbol;
...
if ($r['status'] == 'OK') {
  ...
  if (!empty($data)) {
    ...
    foreach ($data as $row) {
      // $dt = DateTime::createFromFormat('Y-m-d H:i:s.u', $row['DateTime'], $timezoneutc);
      // $ts = $dt->format('Uv');
      // $ask_pts .= '{x:' . $ts . ',y:' . floatval($row['Ask']) . '},';
      // $bid_pts .= '{x:' . $ts . ',y:' . floatval($row['Bid']) . '},';

      $dt_display = DateTime::createFromFormat('Y-m-d H:i:s.u', $row['DateTime'], $timezoneutc);
      $dt_display->setTimezone($timezone);
      $duka_ts = substr($dt_display->format('H:i:s.u'), 0, -3);
      $datax[] = "'" . $duka_ts . "'";
  ...
  print ("ERROR fetching tick data: " . ($r['data'] ?? $r['status']));
}
...
<body>
  <script>
    // Dark theme: default ON, toggled from landing page checkbox
  ...
  <style>
    ...
    .dark-theme .info-bar .duka-badge {
      color: #4caf50;
    }
  ...
  <?php if ($embed !== '1'): ?>
    <div>
      <form action="graph_dukas.php" method="GET" target="_self">
    ...
    <div class="info-bar"
      style="margin: 10px 0; padding: 10px 15px; background: #e8f5e9; border-left: 4px solid #4caf50; border-radius: 6px; font-family: sans-serif;">
      <h2 style="margin: 0 0 6px 0;"><span class="duka-badge"
          style="font-size: 14px; font-weight: bold; margin-right: 8px;">&#x1F4CA; DUKASCOPY</span>
        <?php echo htmlspecialchars($symbol); ?>   <?php if ($resolved_symbol !== $symbol): ?> <span
            style="color: #888; font-size: 14px;">— <?= htmlspecialchars($resolved_symbol) ?></span><?php endif; ?> &mdash;
        <?php echo htmlspecialchars($news_title); ?>
      </h2>
  ...
  $r = get_dukas_data($symbol, $news_timestamp, $dt_from, $dt_to, "m1");

  if (!empty($r['debug'])) {
    debug_to_console("M1 debug: " . implode(' | ', $r['debug']));
  }
  debug_to_console("M1 data count: " . count($r['data'] ?? []));
  ...
  if ($r['status'] == 'OK') {
    ...
    if (!empty($r['data'])) {
      ...
      foreach ($data as $row) {
        $dt_display = DateTime::createFromFormat('Y-m-d H:i:s.u', $row['DateTime'], $timezoneutc);
        $dt_display->setTimezone($timezone);
        $duka_ts = $dt_display->getTimestamp() * 1000; //format('Y-m-d H:i:s');
        $datax[] = "'" . $duka_ts . "'";
        $candles .= '{' . 'x:' . $duka_ts . ', o:' . $row['BidOpen'] . ', h:' . $row['BidHigh'] . ',l:' . $row['BidLow'] . ',c:' . $row['BidClose'] . '},';
        //print($row['DateTime'].">".$news." ".($row['DateTime']>$news)."<br>");
        $n++;
      }
    ...
    print ("ERROR fetching minute data: " . ($r['data'] ?? $r['status']));
  }
  ?>
</body>
